 ...
 */
const DIALOG_STYLES = `dialog:not([open]){display:none!important}dialog{max-inline-size:100vw;max-block-size:100vh;overflow-y:auto}dialog::backdrop{background-color:rgb(0 0 0 / 0.5)}`
...
export interface BuildHtmlInput {
  ...
  /**
   * Studio design-system CSS (variables + runtime bridges) from the DB.
   * Emitted last so it overrides the compiled bundle, mirroring the live
   * site's `<style id="studio-theme">` injection in app/(site)/layout.tsx.
   */
...
export function buildDocument({
  ...
  const css = [fontsCss, colorVariablesCss, publishedCss, studioCss, hasDialog ? DIALOG_STYLES : '']
    .filter(Boolean)
    .join('\n')
