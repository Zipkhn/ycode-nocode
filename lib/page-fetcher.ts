...
async function fetchPageByPathInternal(
  ...
  try {
        ...
        if (extractedSlug) {
          ...
          if (cmsSettings?.collection_id && cmsSettings?.slug_field_id) {
            ...
            let resolvedLayers = layersWithInjectedData.length > 0
              ? await resolveCollectionLayers(layersWithInjectedData, isPublished, enhancedItemValues, paginationContext, translations, collectionItem.id, timezone, pageDataForVisibility, collectionItem.id)
              : [];
    ...
    let resolvedLayers = layersWithComponents.length > 0
      ? await resolveCollectionLayers(layersWithComponents, isPublished, undefined, paginationContext, translations, undefined, timezone, pageDataForVisibility)
      : [];
...
export async function resolveCollectionLayers(
        ...
        try {
          ...
          if (hasStaticFilters) {
            filteredItems = filteredItems.filter(item =>
              evaluateCollectionFilters(staticFilters!, {
  ...
  const filteredResult = filterByVisibility(resultWithPagination, undefined, parentItemValues, currentPageData, pageCollectionItemId ?? parentCollectionItemId, timezone);
...
export async function renderCollectionItemsToHtml(
  ...
  const renderedItems = await Promise.all(
    preprocessed.map(async ({ item, rawValues }, index) => {
      ...
      resolvedLayers = filterByVisibility(resolvedLayers, item.values, undefined, undefined, pageLinkContext?.pageCollectionItemId, htmlTimezone);
