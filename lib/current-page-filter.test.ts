...
import { evaluateCondition, evaluateCollectionFilters, type VisibilityContext } from '@/lib/layer-utils';
...
test('evaluateCollectionFilters end-to-end: single group with a current_page reference condition', () => {
  const filters: ConditionalVisibility = {
    groups: [{ id: 'g1', conditions: [refCondition()] }],
  };
  const keep = evaluateCollectionFilters(filters, ctx({
    collectionLayerData: { 'posts-tags-field': JSON.stringify([TAG_A]) },
    pageCollectionItemId: TAG_A,
  }));
  assert.equal(keep, true);
  const drop = evaluateCollectionFilters(filters, ctx({
    collectionLayerData: { 'posts-tags-field': JSON.stringify([TAG_B]) },
    pageCollectionItemId: TAG_A,
  }));
