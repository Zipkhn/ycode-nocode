...
const CLASS_PROPERTY_MAP: Record<string, RegExp> = {
  ...
  // Spacing
  padding: /^p-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingX: /^px-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingY: /^py-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingTop: /^pt-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingRight: /^pr-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingBottom: /^pb-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  paddingLeft: /^pl-(\[.+\]|\d+|px|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  margin: /^m-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginX: /^mx-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginY: /^my-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginTop: /^mt-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginRight: /^mr-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginBottom: /^mb-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
  marginLeft: /^ml-(\[.+\]|\d+|px|auto|0\.5|1\.5|2\.5|3\.5|space-[a-z0-9-]+)$/,
...
export function propertyToClass(
  ...
  if (category === 'sizing') {
    ...
    if (property === 'gridColumnSpan') {
      ...
      return span === 'full' ? 'u-col-span-full' : `u-col-span-${span}`;
    }

    // Grid Row Span (Studio native)
    if (property === 'gridRowSpan') {
      const span = normalizeGridSpanValue(value);
      return span === 'full' ? 'u-row-span-full' : `u-row-span-${span}`;
    }
  }
...
export function removeConflictsForClass(
  ...
  affectedProperties.forEach(property => {
    ...
    // Scoped to the same breakpoint, consistent with the fork's per-breakpoint conflict removal.
    SPACING_OVERRIDES[property]?.forEach(overridden => {
      result = removeConflictingClassesForBreakpoint(result, overridden, breakpoint);
    });
  });
